$(document).ready(function (event) {
	$('.ui.dropdown.id_pasien').dropdown({
		apiSettings: {
			url: '/administrasi/pegawai/data-pegawai-detail'
		}
	});
	var modal = '#modal-insert',
		form = '#form-insert';
		$(form + ' [name="tgl_lahir"]').popup({inline:true});
	$('[name="tgl_lahir"]').inputmask('dd-mm-yyyy');
	var data_pegawai = null;
	var table = $('#data-pegawai').DataTable({
		ajax: {
			url: $('#data-pegawai').data('source'),
		},
		language: {
			url: $('#data-pegawai').data('lang')
		},
		lengthMenu: [
		[10, 25, 100, 200, 500, -1],
	...
	table.on('init.dt', function () {
		...
		$('#yadcf-filter-wrapper--data-pegawai-0').find('input').css('width', '30px');
		$('#yadcf-filter-wrapper--data-pegawai-1').find('input').css('width', '60px');
		$('#yadcf-filter-wrapper--data-pegawai-2').find('input').css('width', '60px');
		$('#yadcf-filter-wrapper--data-pegawai-3').find('select').css('height', '28px');
		$('#yadcf-filter-wrapper--data-pegawai-4').find('input').css('width', '70px');
		$('#yadcf-filter-wrapper--data-pegawai-5').find('input').css('width', '80px');
		$('#yadcf-filter-wrapper--data-pegawai-6').find('input').css('width', '80px');
		$('#yadcf-filter-wrapper--data-pegawai-7').find('select').css('height', '28px');
		new $.fn.dataTable.Buttons(table, {
			...
			buttons: [{
				...
				action: function (e, dt, node, config) {
					$(modal + ' .header').text('Tambah Data Pegawai');
					$(modal).modal({
						closable: false,
						onApprove: function () {
		...
		new $.fn.dataTable.Buttons(table, {
			...
			buttons: [{
				...
				className: 'check-btn',
				action: function (e, dt, node, config) {
					$('#data-pegawai .checkbox-table').checkbox('check');
				}
			}, {
				text: '<i class="icon close"></i>Un-Check',
				className: 'check-btn',
				action: function (e, dt, node, config) {
					$('#data-pegawai .checkbox-table').checkbox('uncheck');
				}
			}, {
	...
	$('#data-pegawai').on('click', '.edit-btn', function(e){
		e.preventDefault();
		table.ajax.reload();
	...
	$('#data-pegawai').on('click', '.view-btn', function(e){
		e.preventDefault();
		var data = table.row($(this).parents('tr')).data();
		if (!data) {
			return false;
		}
		$('#modal-view .header').text(data.jabatan + ': '+ data.nama);
