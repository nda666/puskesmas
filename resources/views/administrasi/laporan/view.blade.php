@extends('administrasi.default') @section('title','Rawat Jalan Pasien: '.$rawat_jalan->pasien->nama) @section('content_title','Rawat Jalan Pasien') @section('content_sub_title', 'Management Konsultasi, Resep & Obat Pasien') @section('content_title_icon')
<i class="big icons">
  <i class="icon user"></i>
...
<script type="text/javascript" src="{{ asset('/assets/js/administrasi/view-laporan.min.js') }}"></script>
<script type="text/javascript" src="{{ asset('/assets/js/administrasi/view-laporan-tindakan.min.js') }}"></script>
<script type="text/javascript" src="{{ asset('/assets/js/administrasi/view-laporan-resep.min.js') }}"></script>
...
<div class="sixteen wide column no bottom padding">
  <div id="tab-rawat-jalan" class="ui stackable five item menu">
    ...
    <a class="link green item" data-token="{{ csrf_token() }}" data-id="{{ $rawat_jalan->id }}" data-action="{{ url('/administrasi/rawat-jalan/selesai') }}" data-progres="1" data-force="false" id="selesai-rawat-jalan"><i class="check icon"></i>Selesai Rawat Jalan</a> @else
    <a class="link red item" data-action="{{ url('/administrasi/rawat-jalan/selesai') }}" data-token="{{ csrf_token() }}" data-id="{{ $rawat_jalan->id }}" data-progres="0" data-force="true" id="selesai-rawat-jalan"><i class="close icon"></i>Batalkan Selesai</a> @endif
  </div>
  ...
  <div class="ui active tab vertical segment" data-tab="data-konsultasi">
    <div class="datatable-container">
      <table id="data-konsultasi" class="ui unstackable celled table" data-lang="{{ asset('/assets/datatables/i18n/indonesian.json')}}" data-id="{{ $rawat_jalan->id }}" data-token="{{ csrf_token() }}" width="100%"  data-source="{{ url('administrasi/laporan/data-konsultasi/'.$rawat_jalan->id) }}">
        <thead>
          <tr>
            <th>ID</th>
      ...
      @include('administrasi.laporan.view.konsultasi')
  </div>

  {{-- Data Tindakan --}}
  <div class="ui tab vertical segment" data-tab="data-tindakan">
      @include('administrasi.laporan.view.tindakan')
  </div>

  {{-- Data Resep --}}
  <div class="ui tab vertical segment" data-tab="data-resep">
    <div class="datatable-container">
      <table id="data-konsultasi-resep" class="ui celled table" data-lang="{{ asset('/assets/datatables/i18n/indonesian.json')}}" data-pasien="{{$rawat_jalan->pasien->id}}" data-token="{{ csrf_token() }}" width="100%" data-source="{{ url('administrasi/laporan/data-resep/'.$rawat_jalan->id) }}">
        <thead>
        <tr>
          <th>ID</th>
          <th>Resep</th>
